...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    var window: UIWindow?
    var environment:ArcEnvironment = EXEnvironment.production
...
//		Arc.shared.appController.language = "en"
//		Arc.shared.appController.country = "US"
//		Arc.shared.setLocalization(country:"US",
//								   language: "en",
//								   shouldTranslate: true)
		...
		NotificationCenter.default.addObserver(forName: .ACMigrationTrigger, object: nil, queue: .main) {(notif) in
			if Arc.shared.appController.flags["migrated_legacy"] != true {
				EXCoreDataStack.shared.migrate {
					result in
					
					//If we flagged migration as started, then report that it is finished.
					if Arc.shared.appController.flags["migration_in_progress"] != nil {
						Arc.shared.appController.flags["migration_in_progress"] = false
						
					} else {
						//Otherwise don't set any migration flags. This can create undesired behavior.
						return
					}
					switch result {
					case .success(_):
						Arc.shared.appController.flags["migrated_legacy"] = true
						NotificationCenter.default.post(name: .ACMigrationSuccessful, object: nil)
						NotificationCenter.default.post(name: .ACSessionUploadComplete, object: Arc.shared.sessionController.sessionUploads)
						break
					case .failure(let error):
						Arc.shared.appController.flags["migrated_legacy"] = false
						NotificationCenter.default.post(name: .ACMigrationFailure, object: error)
...
//			Arc.shared.appController.isNotificationAuthorized = $0
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
		...
		{
            // if last closed is less than 5 minutes
		...
		if let state = Arc.currentState as? EXState, [EXState.onboarding, EXState.auth, EXState.schedule, EXState.changeSchedule].contains(state) && Arc.shared.appController.commitment != .rebuked {
...
//            self.backgroundTask = nil
    ...
    {
        HMLog("-=-==-==-=-=-=-=-=-=-=-Not presenting notification");
		...
		Arc.shared.displayAlert(message: message,
			options: [.default("CONFIRM".localized(ACTranslationKey.button_confirm), {}),
					  .default("ADJUST SCHEDULE".localized(ACTranslationKey.button_adjustschedule), {
						Arc.shared.appNavigation.navigate(state: EXState.changeStudyStart, direction: .toRight)
