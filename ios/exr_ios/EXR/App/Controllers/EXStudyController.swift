...
class EXStudyController : StudyController {
...
//                return .long
				...
				//We are not performing remaining day length shortening for EXR
	...
	open override func createStudyPeriod(forDate: Date, studyId:Int) -> StudyPeriod
	{
		...
		let phase = EXPhase.from(studyId: studyId)
	...
	override open func create<T:Phase>(testSessionsWithSchedule schedule:TestScheduleRequestData, with PhaseType:T.Type) -> Bool where T.PhasePeriod == T{
		...
		MHController.dataContext.performAndWait {
            ...
            let cycleProgressMap = schedule.cycle_progress?.asDictionary
			...
			for sessionData in schedule.sessions
			{
                ...
                // uploading finished and missed sessions depending on status
                if let status_value = cycleProgressMap?[sessionID] {
                    if status_value == "completed"{
                        session.finishedSession = true
                        session.uploaded = true
                    } else if status_value == "incomplete" {
	...
	override open func createTestSessions(studyId: Int, isRescheduling:Bool = false, now:Date = Date())
    {
        ...
        let startingSessionId = EXPhase.startingSessionId(forStudyId: studyId);
    ...
    private func addTestStatesForSession(_ session: Session, phase: EXPhase)
    {
        let states = phase.statesForSession(week: Int(session.week),
                                            day: Int(session.day),
                                            session: Int(session.session))
