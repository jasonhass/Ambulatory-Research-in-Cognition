...
import CoreData
@testable import Arc
@testable import MAP_ARC

enum HASDTestError : Error {
...
class HASDTests: XCTestCase {
	let environment = HASDEnvironment.unitTest
	...
	func testScheduleStartsSequentialIds() {
		MHController.dataContext.performAndWait {
			...
			let output = values.map{"\(($0.sessionID, $0.sessionDate!.localizedFormat(), $0.study!.studyID))"}
			for o in output {
				print(o)
			}
			if errors.count > 0 {
	...
	func testScheduleFourSessionsADay() {
		...
		MHController.dataContext.performAndWait {
			let startDate =  time(year: 2020, month: 1, day: 1, hour: 8, minute: 0)
			let _ = generateSchedule(start: time(year: 2020, month: 1, day: 1, hour: 18, minute: 30),
	...
	func testSessionCreation() {
		...
		MHController.dataContext.performAndWait {
			let startDate = time(year: 2019, month: 6, day: 26, hour: 0, minute: 0)
			let _ = generateSchedule(start: time(year: 2020, month: 1, day: 1, hour: 8, minute: 0),
											end: time(year: 2020, month: 1, day: 1, hour: 20, minute: 0))
