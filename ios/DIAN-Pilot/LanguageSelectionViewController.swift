...
import UIKit
private struct LanguageData : ACCellData {
    var name:String
...
class LanguageSelectionViewController: ACCellSelectionViewController {

    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
        data = DNDataManager.sharedInstance.availableLanguages.map({ (lang) -> ACCellData in
            let d = LanguageData(name: lang, isSelected: false)
            return d
    ...
    */
    @IBAction override func savePressed(_ sender: Any) {
        //Not calling super here
        guard let selection = selected.first?.row else {
            return
        }
        let language = data[selection].getTitle()
        DNDataManager.sharedInstance.languageKey = language
        AppDelegate.go(state: .setupUser)
    }
}
