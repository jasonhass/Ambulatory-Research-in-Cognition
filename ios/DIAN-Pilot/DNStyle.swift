...
class DNStyle {
    static func applyStyle() {
        let buttonLabel = UILabel.appearance(whenContainedInInstancesOf: [DNButton.self])
        buttonLabel.adjustsFontSizeToFitWidth = true
        buttonLabel.layoutMargins = UIEdgeInsets(top: 0, left: 8, bottom: 0, right: 8)

        let weekdayButtonLabel = UILabel.appearance(whenContainedInInstancesOf: [WeekdayButton.self])
        weekdayButtonLabel.adjustsFontSizeToFitWidth = false
        weekdayButtonLabel.layoutMargins = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
