...
class ResendCodeViewController: DNViewController {
    ...
    @IBAction func resendCode(_ sender: Any) {
        DNRestAPI.shared.requestVerificationCode(arcId: "\(DNDataManager.sharedInstance.arcId!)", onCompletion: { (error) in
            if error != nil
            {
              dump(error)
            }
