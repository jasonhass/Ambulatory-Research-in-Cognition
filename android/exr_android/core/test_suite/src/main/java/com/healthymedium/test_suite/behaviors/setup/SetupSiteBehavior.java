...
public class SetupSiteBehavior extends Behavior {

    @Override
    public void onOpened(BaseFragment fragment){
        super.onOpened(fragment);

        String siteId = "123456";
        if(siteId==null){
            siteId = String.format("%06d",0);
        }

        ViewInteraction root = Espresso.onView(ViewMatchers.isRoot());
        int length = siteId.length();
        for(int i = 0; i<length; i++){
            char character = siteId.charAt(i);
